...
export function RequestList() {
  ...
  useEffect(() => {
    ...
    const fetchRequests = async () => {
      try {
        const allRequests = await RequestService.getRequestsByActor(user?.id || '');
  ...
  const createAcceptedEHR = async (request: Request) => {
    try {
      ...
      await EHRService.updatePatientEHRID(request.patientMatricule, parseInt(ehrId), user.id,orgName);
